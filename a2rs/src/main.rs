...
use profiler::{Profiler, Debugger, DebuggerState, ProfileCategory};
use clap::Parser;
...
fn main() {
    ...
    let model = match args.model.to_lowercase().as_str() {
        "auto" => {
            if let Some(ref data) = rom_data {
                let detected = Apple2::detect_model_from_rom(data);
                println!("Auto-detected model from ROM size ({} bytes): {:?}", data.len(), detected);
                detected
            } else {
    ...
    println!("A2RS - Apple II Emulator in Rust v0.1");
    println!("========================");
    println!("Model: {:?}", model);
        ...
        match fs::read(&disk_rom_path) {
            Ok(data) => {
                match emu.load_disk_rom(&data) {
                    Ok(()) => {
                        println!("Loaded Disk II Boot ROM: {} ({} bytes)", disk_rom_path, data.len());
                        true
                    }
                    Err(e) => {
        ...
        for path in &search_paths {
            if let Ok(data) = fs::read(path) {
                if let Ok(()) = emu.load_disk_rom(&data) {
                    println!("Loaded Disk II Boot ROM: {} ({} bytes)", path, data.len());
                    loaded = true;
                    break;
                }
            }
    ...
    if !disk_rom_loaded {
        println!("Note: Disk II Boot ROM not loaded.");
        println!("      VBR (Virtual Boot ROM) mode will be used for DSK files.");
        println!("      For NIB/WOZ files, please provide --disk-rom option.");
    }

    // ROMをロード
    if let Some(data) = rom_data {
        emu.load_rom(&data);
        println!("Loaded ROM: {} ({} bytes)", args.rom.as_ref().unwrap(), data.len());
    } else {
        // テスト用ROMを使用
        println!("No ROM specified. Loading built-in test ROM...");
        let test_rom = apple2::create_test_rom();
        emu.load_rom(&test_rom);
        println!("Test ROM loaded. You should see 'APPLE II EMULATOR' on screen.");
    }
        ...
        match fs::read(&disk1_path) {
            Ok(disk_data) => {
                match emu.load_disk(0, &disk_data) {
                    Ok(()) => println!("Loaded disk 1: {}", disk1_path),
        ...
        match fs::read(&disk2_path) {
            Ok(disk_data) => {
                match emu.load_disk(1, &disk_data) {
                    Ok(()) => println!("Loaded disk 2: {}", disk2_path),
    ...
    // ディスク高速化を設定（デフォルトはオン）
    // --fast-disk オプションで明示的に有効化した場合も同じ
    // 既にデフォルトで enhance_disk=true なので、明示的に確認のため設定
    emu.set_fast_disk(true);
    if args.fast_disk {
        println!("Fast disk mode enabled");
    }
    
    // デバッグ情報を表示
    println!("\nDebug Info:");
    println!("  Reset vector address: ${:04X}", emu.cpu.regs.pc);
    println!("  ROM size: {} bytes", emu.memory.rom.len());
    println!("  Text mode: {}", emu.memory.switches.text_mode);
    println!("  Model: {:?}", emu.memory.model);
    
    // ROMの最後の16バイトを表示（ベクタ領域）
    if emu.memory.rom.len() >= 16 {
        let start = emu.memory.rom.len() - 16;
        print!("  ROM vectors ($FFF0-$FFFF): ");
        for i in 0..16 {
            print!("{:02X} ", emu.memory.rom[start + i]);
        }
        println!();
    }
    ...
    for i in 0..10 {
        ...
        let byte1 = emu.read(pc.wrapping_add(1));
        let byte2 = emu.read(pc.wrapping_add(2));
        println!("  {:2}: ${:04X}: {:02X} {:02X} {:02X}  A={:02X} X={:02X} Y={:02X} S={:02X} P={:02X}",
                 i, pc, opcode, byte1, byte2,
                 emu.cpu.regs.a, emu.cpu.regs.x, emu.cpu.regs.y, 
                 emu.cpu.regs.sp, emu.cpu.regs.status);
        emu.step();
    }
    
    // 1フレーム分実行してからテキストメモリを確認
    ...
    println!("  Text mode: {}", emu.memory.switches.text_mode);
    println!("  Hires mode: {}", emu.memory.switches.hires);
    println!("  Mixed mode: {}", emu.memory.switches.mixed_mode);
    println!("  Page2: {}", emu.memory.switches.page2);
    
    // テキストメモリの最初の行を表示
    print!("\nText memory $0400-$0427 (row 0): ");
    for i in 0..40 {
        let ch = emu.memory.main_ram[0x0400 + i];
        print!("{:02X} ", ch);
    }
    println!();
    ...
    print!("  As text: ");
    for i in 0..40 {
        let ch = emu.memory.main_ram[0x0400 + i] & 0x7F;
        if ch >= 0x20 && ch < 0x7F {
            print!("{}", ch as char);
        } else {
            print!(".");
    ...
    print!("\nFont ROM (char $00 '@'): ");
    for i in 0..8 {
        print!("{:02X} ", emu.video.char_rom[i]);
    }
    println!();
    print!("Font ROM (char $01 'A'): ");
    for i in 8..16 {
        print!("{:02X} ", emu.video.char_rom[i]);
    }
    println!();
    
    // Apple IIe の場合、追加でHi-Resメモリも確認
    if matches!(emu.memory.model, AppleModel::AppleIIe | AppleModel::AppleIIeEnhanced) {
        print!("\nHi-Res memory $2000-$200F: ");
        for i in 0..16 {
            print!("{:02X} ", emu.memory.main_ram[0x2000 + i]);
        }
        println!();
    }
    println!();
    ...
    if args.headless {
        ...
        run_with_window(&mut emu, args.speed, args.fast, width, height, args.fullscreen, profile_opts);
    }
}
...
fn run_headless(emu: &mut Apple2, cycles: u64) {
    println!("Running {} cycles in headless mode...", cycles);
    ...
    println!("CPU PC: ${:04X}", emu.cpu.regs.pc);
}
...
fn run_with_window(emu: &mut Apple2, speed: u32, fast_boot: bool, init_width: usize, init_height: usize, fullscreen: bool, profile_opts: ProfileOptions) {
    // 初期ウィンドウサイズ
    ...
    if profile_opts.enabled {
        ...
        if let Some(ref path) = profile_output {
            ...
            println!("  Output interval: {}s", profile_opts.interval);
        }
        if profile_boot_only {
            println!("  Mode: Boot profiling (will exit after boot)");
        }
        // デバッガパネルも自動で表示
    ...
    println!("\nControls:");
    println!("  ESC - Settings overlay");
    println!("  F1  - Speed (x1/x2/x5/x10/MAX)");
    println!("  F2  - Fast disk toggle");
    println!("  F3  - Quality toggle");
    println!("  F5  - Quick save");
    println!("  F9  - Quick load");
    println!("  F10 - Screenshot");
    println!("  F11 - Toggle fullscreen");
    println!("  F12 - Reset");
    println!("  Tab - Toggle debugger panel");
    println!("");
    
    println!("Debugger:");
    println!("  F6  - Step instruction");
    println!("  F7  - Continue");
    println!("  F8  - Break/Pause");
    println!("");
    
    println!("Quality levels:");
    println!("  0: Lowest  - Nearest neighbor scaling only");
    println!("  1: Low     - Bilinear scaling");
    println!("  2: Medium  - Bilinear + frame blend");
    println!("  3: High    - Bilinear + frame blend + sharpen + scanlines");
    println!("  4: Ultra   - All effects + bloom");
    println!("");
    
    // 高速ブートモードの場合、最初に50Mサイクル実行
    if fast_boot {
        println!("Fast boot: Running 50M cycles...");
        let start = Instant::now();
        emu.run_cycles(50_000_000);
        let elapsed = start.elapsed();
        println!("Fast boot completed in {:?}", elapsed);
    }
    ...
    let mut audio_output = match AudioOutput::new() {
        ...
        Err(e) => {
            println!("Warning: Audio initialization failed: {}", e);
            None
        }
    };
    ...
    let mut gamepad_manager = match GamepadManager::new() {
        Ok(gp) => Some(gp),
        Err(e) => {
            println!("Gamepad not available: {}", e);
            None
        }
    };
        ...
        if last_fps_update.elapsed() >= Duration::from_secs(1) {
            ...
            if auto_quality {
                ...
                if old_quality != quality_level {
                    ...
                    println!("Auto quality: {} (FPS: {:.1})", quality_name, displayed_fps);
                }
            }
    ...
    println!("Total cycles: {}", emu.total_cycles);
    println!("Total frames: {}", emu.frame_count);
}
