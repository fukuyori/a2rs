...
///
/// 現在は既存の命令単位実行器の上に載せたハイブリッド実装であり、
/// 命令開始時に総サイクル数を確定し、以降の tick ごとに残サイクルを
/// 消費しながら bus / video / disk と同期する。
#[derive(Debug, Clone, Copy)]
pub struct CpuMicroState {
    /// 現在のオペコード。
    pub opcode: u8,
    /// 命令開始からのマイクロサイクル番号 (0-based)。
    pub micro_cycle: u8,
    /// 命令完了までの残りCPUサイクル数。
    pub remaining_cycles: u8,
    /// 命令全体のサイクル数。
    pub total_cycles: u8,
    /// 直近の有効アドレス（将来の floating bus / trace 用）。
    pub effective_address: u16,
    /// 直近のデータ値（将来の trace / nibble timing 用）。
    pub data: u8,
}
...
impl Cpu {
    ...
    pub fn reset<M: MemoryBus>(&mut self, memory: &mut M) {
        ...
        self.total_cycles += 7;
        self.micro = CpuMicroState {
            opcode: 0x00,
            micro_cycle: 0,
            remaining_cycles: 0,
            total_cycles: 7,
            effective_address: self.regs.pc,
            data: 0,
        };
    }
    ...
    pub fn step<M: MemoryBus>(&mut self, memory: &mut M) -> u32 {
        ...
        self.micro = CpuMicroState {
            opcode,
            micro_cycle: 0,
            remaining_cycles: self.cycles.saturating_sub(1) as u8,
            total_cycles: self.cycles.min(u8::MAX as u32) as u8,
            effective_address: self.regs.pc,
            data: self.regs.a,
        };
...
impl Cpu {
    /// 現在マイクロサイクル実行中かどうか。
    #[inline]
    pub fn in_microcycle(&self) -> bool {
        self.micro.remaining_cycles > 0
    }

    /// 1 CPU cycle 分だけマイクロサイクル状態を前進させる。
    ///
    /// 命令デコード自体は既存の命令実行器が担当し、ここでは
    /// システム tick 用に cycle-by-cycle の可視状態だけを更新する。
    #[inline]
    pub fn advance_microcycle(&mut self) {
        if self.micro.remaining_cycles > 0 {
            self.micro.remaining_cycles -= 1;
            self.micro.micro_cycle = self.micro.micro_cycle.saturating_add(1);
        }
    }
}
