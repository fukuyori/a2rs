...
use crate::disk::{
    Disk2InterfaceCard, DiskControllerState, DiskFormat, DiskSequencerMode, DSK_SIZE, NIB_SIZE,
};
use crate::memory::{AppleModel, Memory};
use crate::savestate::{CpuState, DiskDriveState, DiskState, MemoryState, SaveState, VideoState};
use crate::system::{
    CYCLES_PER_FRAME as SYSTEM_CYCLES_PER_FRAME, CYCLES_PER_SCANLINE as SYSTEM_CYCLES_PER_SCANLINE,
};
...
pub struct Apple2 {
    ...
    /// 現在の命令内で発生したバスアクセス数。
    ///
    /// CPU 本体はまだ命令単位実行だが、Disk II はラッチ読み取り時刻に敏感なため、
    /// 命令内の何サイクル目に I/O アクセスしたかをここで近似する。
    cpu_bus_cycle_offset: u32,
    /// Disk II の bit timing をここまで進めたシステムサイクル。
    disk_ticked_until_cycle: u64,
}

/// メモリバスの実装（Disk II I/Oを含む）
impl MemoryBus for Apple2 {
    fn read(&mut self, address: u16) -> u8 {
        let access_cycle = self.note_cpu_bus_access();
        match address {
            // スピーカー ($C030-$C03F)
            ...
            0xC0E0..=0xC0EF => {
                // 命令内の実アクセス時刻まで Disk II を追いつかせてから I/O を実行する。
                self.sync_disk_to_cycle(access_cycle);
...
impl Apple2 {
    ...
    pub fn new(model: AppleModel) -> Self {
        ...
        Apple2 {
            ...
            disk_ticked_until_cycle: 0,
        }
    }

    #[inline]
    fn note_cpu_bus_access(&mut self) -> u64 {
        let access_cycle = self
            .total_cycles
            .saturating_add(self.cpu_bus_cycle_offset as u64);
        self.cpu_bus_cycle_offset = self.cpu_bus_cycle_offset.saturating_add(1);
        access_cycle
    }

    #[inline]
    fn begin_cpu_instruction_bus_timing(&mut self) {
        self.cpu_bus_cycle_offset = 0;
    }

    fn sync_disk_to_cycle(&mut self, target_cycle: u64) {
        while self.disk_ticked_until_cycle < target_cycle {
            self.disk_ticked_until_cycle = self.disk_ticked_until_cycle.saturating_add(1);
            self.disk.cumulative_cycles = self.disk_ticked_until_cycle;
            self.disk.tick_disk_bit();
        }
        self.disk.cumulative_cycles = target_cycle;
    }
    ...
    pub fn load_disk(&mut self, drive: usize, data: &[u8]) -> Result<(), &'static str> {
        ...
        if woz::is_woz(data) {
            ...
            self.disk
                .insert_disk(drive, &result.nib_data, DiskFormat::Woz)?;
    ...
    pub fn reset(&mut self) {
        ...
        self.cpu_bus_cycle_offset = 0;
        self.disk_ticked_until_cycle = 0;
        self.disk.cumulative_cycles = 0;
    ...
    pub fn step(&mut self) -> u32 {
        ...
        self.disk
            .observe_pc_with_memory(self.cpu.regs.pc, &self.memory.main_ram[..]);

        // CPUを一時的に取り出して実行
        self.begin_cpu_instruction_bus_timing();
        let mut cpu = std::mem::take(&mut self.cpu);
        let cycles = cpu.step(self);
        self.cpu = cpu;
        self.total_cycles += cycles as u64;
        self.sync_disk_to_cycle(self.total_cycles);
    ...
    fn record_pc(&mut self, pc: u16) {
        ...
        if zone != self.last_pc_zone {
            ...
            if self.boost_log {
                println!(
                    "[PC-ZONE] {} -> {} at ${:04X} t={:.2}M",
                    zone_name(self.last_pc_zone),
                    zone_name(zone),
                    pc,
    ...
    pub fn check_stable_loop(&mut self) -> bool {
        ...
        let user_ratio = if valid_count > 0 {
            user_space_count as f32 / valid_count as f32
        } else {
            0.0
        };
        let disk_rom_ratio = if valid_count > 0 {
            disk_rom_count as f32 / valid_count as f32
        } else {
            0.0
        };
        ...
        let disk_io_silence = self
            .total_cycles
            .saturating_sub(self.disk.last_disk_io_cycle);
        ...
        if self.boost_log && (self.total_cycles % 1_000_000 < 100_000 || is_candidate) {
            ...
            println!(
                "[BOOST-STATE] user_entered={} zone={} disk_left={:.2}M io_silence={:.2}M motor={}",
                self.user_ram_entered,
                match self.last_pc_zone {
                    0 => "zero",
                    1 => "user",
                    2 => "disk_rom",
                    3 => "main_rom",
    ...
    fn scroll_text(&mut self) {
        ...
        for y in 0..23 {
            ...
            for x in 0..40 {
                self.memory.main_ram[(dst + x) as usize] = self.memory.main_ram[(src + x) as usize];
            }
        }
        // 最終行をクリア
    ...
    fn vbr_boot(&mut self) -> bool {
        ...
        // DSK は保持セクタから、NIB/WOZ は物理ニブル列からブートセクタを復元する。
        let sector_data = match self.disk.read_sector_for_boot(0, 0, 0) {
            Some(data) => data,
            None => return false,
        };
    ...
    fn generate_decode_table(&mut self) {
        ...
        let decode_table: [u8; 64] = [
            0x00, 0x01, 0xFF, 0xFF, 0x02, 0x03, 0xFF, 0x04, 0x05, 0x06, 0xFF, 0xFF, 0xFF, 0xFF,
            0xFF, 0xFF, 0x07, 0x08, 0xFF, 0xFF, 0xFF, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0xFF, 0xFF,
            0x0E, 0x0F, 0x10, 0x11, 0x12, 0x13, 0xFF, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A,
            0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0x1B, 0xFF, 0x1C,
    ...
    fn text_address_for_scanline(&self, row: usize, col: usize) -> u16 {
        let base: usize = if self.memory.switches.page2 {
            0x0800
        } else {
            0x0400
        };
        let char_row = (row / 8).min(23);
        let addr = base + ((char_row & 0x07) << 7) + ((char_row >> 3) * 40) + col.min(39);
        addr as u16
    }
    ...
    pub fn save_state(&self) -> SaveState {
        SaveState {
            ...
            memory: MemoryState {
                ...
                aux_bank2: self.memory.aux_lc_ram_bank2.to_vec(),
                aux_lc_ram: self.memory.aux_lc_ram.to_vec(),
            ...
            disk: DiskState {
                ...
                woz_bit_cycle_accumulator: self.disk.woz_bit_cycle_accumulator,
    ...
    pub fn load_state(&mut self, state: &SaveState) -> Result<(), &'static str> {
        ...
        if state.memory.bank2.len() == self.memory.lc_ram_bank2.len() {
            self.memory
                .lc_ram_bank2
                .copy_from_slice(&state.memory.bank2);
        } else if state.memory.bank1.len() == self.memory.lc_ram_bank2.len() {
            self.memory
                .lc_ram_bank2
                .copy_from_slice(&state.memory.bank1);
        }
        if state.memory.lc_ram.len() == self.memory.lc_ram.len() {
            self.memory.lc_ram.copy_from_slice(&state.memory.lc_ram);
        }
        if state.memory.aux_bank2.len() == self.memory.aux_lc_ram_bank2.len() {
            self.memory
                .aux_lc_ram_bank2
                .copy_from_slice(&state.memory.aux_bank2);
        }
        if state.memory.aux_lc_ram.len() == self.memory.aux_lc_ram.len() {
            self.memory
                .aux_lc_ram
                .copy_from_slice(&state.memory.aux_lc_ram);
        }
        ...
        if state.version >= 3 {
            self.disk
                .set_motor_off_scheduled_cycle_for_restore(state.disk.motor_off_scheduled_cycle);
        } else {
        ...
        self.disk.woz_bit_cycle_accumulator = state.disk.woz_bit_cycle_accumulator;
        ...
        self.cpu_bus_cycle_offset = 0;
        self.disk_ticked_until_cycle = self.total_cycles;
        self.disk.cumulative_cycles = self.total_cycles;
