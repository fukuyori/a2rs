//! Apple II サウンドエミュレーション (1bit Speaker)
//!
//! Apple II のスピーカーは $C030 アクセスで極性が反転する 1bit 出力である。
//! ここでは click 時刻列からサンプル区間平均を生成し、
//! DC blocker で無音時の唸りを抑えつつ PCM 化する。

use std::collections::VecDeque;

#[cfg(feature = "audio")]
use std::sync::atomic::{AtomicUsize, Ordering};

/// サンプルレート (Hz)
pub const SAMPLE_RATE: u32 = 44100;

/// 1フレームあたりのサンプル数 (44100 / 60)
const SAMPLES_PER_FRAME: usize = 735;

/// リングバッファサイズ（約0.2秒分）
#[cfg(feature = "audio")]
const RING_BUFFER_SIZE: usize = 8192;

/// 軽いローパス（耳に刺さる高域を少し丸める）
struct LowPass {
    alpha: f32,
    z: f32,
}

impl LowPass {
    fn new(cutoff_hz: f32, sample_rate: f32) -> Self {
        let rc = 1.0 / (2.0 * std::f32::consts::PI * cutoff_hz);
        let dt = 1.0 / sample_rate;
        let alpha = dt / (rc + dt);
        Self { alpha, z: 0.0 }
    }

    fn process(&mut self, input: f32) -> f32 {
        self.z += self.alpha * (input - self.z);
        self.z
    }
}

/// Apple IIスピーカーエミュレータ (1bit方式)
pub struct Speaker {
    /// クリックイベントキュー (サイクル数)
    click_queue: VecDeque<u64>,
    /// サウンドが有効か
    enabled: bool,
    /// ボリューム（0.0 - 1.0）
    volume: f32,
    /// 現在のスピーカー状態 (true = HIGH, false = LOW)
    speaker_state: bool,
    /// サンプル生成用バッファ（再利用）
    sample_buffer: Vec<f32>,
    /// ローパスフィルタ
    lpf: LowPass,
    /// 最後に処理したサイクル
    last_processed_cycle: u64,
    /// DC blocker の前回入力
    dc_prev_in: f32,
    /// DC blocker の前回出力
    dc_prev_out: f32,
}

impl Speaker {
    pub fn new() -> Self {
        Speaker {
            click_queue: VecDeque::with_capacity(4096),
            enabled: true,
            volume: 0.25,
            speaker_state: false,
            sample_buffer: vec![0.0; SAMPLES_PER_FRAME],
            lpf: LowPass::new(4500.0, SAMPLE_RATE as f32),
            last_processed_cycle: 0,
            dc_prev_in: 0.0,
            dc_prev_out: 0.0,
        }
    }

    /// リセット音は現在使わない
    pub fn trigger_reset_sound(&mut self) {}

    /// UIクリック音は現在使わない
    pub fn trigger_ui_click(&mut self) {}

    /// スピーカーをクリック（$C030アクセス時に呼ばれる）
    pub fn click(&mut self, cycle: u64) {
        self.click_queue.push_back(cycle);
        if self.click_queue.len() > 8192 {
            self.click_queue.pop_front();
        }
    }

    /// サウンドの有効/無効を設定
    pub fn set_enabled(&mut self, enabled: bool) {
        self.enabled = enabled;
    }

    /// ボリュームを設定（0.0 - 1.0）
    #[allow(dead_code)]
    pub fn set_volume(&mut self, volume: f32) {
        self.volume = volume.clamp(0.0, 1.0);
    }

    fn level_value(state: bool) -> f32 {
        if state { 1.0 } else { -1.0 }
    }

    /// [sample_start, sample_end) 区間の平均レベルを求める。
    fn sample_average_for_interval(&mut self, sample_start: u64, sample_end: u64) -> f32 {
        let mut cursor = sample_start;
        let mut state = self.speaker_state;
        let mut area = 0.0f32;

        while let Some(&click_cycle) = self.click_queue.front() {
            if click_cycle > sample_end {
                break;
            }

            if click_cycle > cursor {
                let dt = (click_cycle - cursor) as f32;
                area += Self::level_value(state) * dt;
                cursor = click_cycle;
            }

            self.click_queue.pop_front();
            state = !state;
        }

        if sample_end > cursor {
            let dt = (sample_end - cursor) as f32;
            area += Self::level_value(state) * dt;
        }

        self.speaker_state = state;
        let span = (sample_end.saturating_sub(sample_start)).max(1) as f32;
        area / span
    }

    /// オーディオサンプルを生成
    pub fn generate_samples(&mut self, base_cycle: u64, cycles_per_frame: u64) -> Option<&[f32]> {
        if !self.enabled || cycles_per_frame == 0 {
            return None;
        }

        let end_cycle = base_cycle + cycles_per_frame;
        let has_clicks = self.click_queue.iter().any(|&c| c <= end_cycle);
        let tail_active = self.dc_prev_out.abs() > 0.0005 || self.lpf.z.abs() > 0.0005;
        if !has_clicks && !tail_active {
            self.last_processed_cycle = end_cycle;
            return None;
        }

        let cycles_per_sample = cycles_per_frame as f32 / SAMPLES_PER_FRAME as f32;
        let dc_r = 0.995f32;
        let mut any_nonzero = false;

        for i in 0..SAMPLES_PER_FRAME {
            let sample_start = base_cycle + (i as f32 * cycles_per_sample) as u64;
            let sample_end = if i + 1 == SAMPLES_PER_FRAME {
                end_cycle
            } else {
                base_cycle + ((i + 1) as f32 * cycles_per_sample) as u64
            };

            let raw = self.sample_average_for_interval(sample_start, sample_end);

            // DC blocker: y[n] = x[n] - x[n-1] + R * y[n-1]
            let hp = raw - self.dc_prev_in + dc_r * self.dc_prev_out;
            self.dc_prev_in = raw;
            self.dc_prev_out = hp;

            let filtered = self.lpf.process(hp);
            let sample = filtered * self.volume;
            any_nonzero |= sample.abs() > 0.0005;
            self.sample_buffer[i] = sample;
        }

        self.last_processed_cycle = end_cycle;

        if any_nonzero || has_clicks {
            Some(&self.sample_buffer)
        } else {
            None
        }
    }

    #[allow(dead_code)]
    pub fn clear(&mut self) {
        self.click_queue.clear();
        self.speaker_state = false;
        self.lpf.z = 0.0;
        self.last_processed_cycle = 0;
        self.dc_prev_in = 0.0;
        self.dc_prev_out = 0.0;
    }
}

impl Default for Speaker {
    fn default() -> Self {
        Self::new()
    }
}

// ============================================================
// オーディオ出力（rodioが有効な場合のみ）
// ============================================================

#[cfg(feature = "audio")]
use rodio::{OutputStream, Sink, Source};

#[cfg(feature = "audio")]
pub struct AudioOutput {
    _stream: OutputStream,
    sink: Sink,
    ring_buffer: std::sync::Arc<RingBuffer>,
}

#[cfg(feature = "audio")]
struct RingBuffer {
    data: Box<[f32; RING_BUFFER_SIZE]>,
    write_pos: AtomicUsize,
    read_pos: AtomicUsize,
}

#[cfg(feature = "audio")]
impl RingBuffer {
    fn new() -> Self {
        RingBuffer {
            data: Box::new([0.0; RING_BUFFER_SIZE]),
            write_pos: AtomicUsize::new(0),
            read_pos: AtomicUsize::new(0),
        }
    }

    fn write(&self, samples: &[f32]) {
        let mut write_pos = self.write_pos.load(Ordering::Relaxed);
        let read_pos = self.read_pos.load(Ordering::Acquire);

        for &sample in samples {
            let next_pos = (write_pos + 1) % RING_BUFFER_SIZE;
            if next_pos == read_pos {
                break;
            }
            unsafe {
                let ptr = self.data.as_ptr() as *mut f32;
                *ptr.add(write_pos) = sample;
            }
            write_pos = next_pos;
        }
        self.write_pos.store(write_pos, Ordering::Release);
    }

    fn read(&self) -> f32 {
        let write_pos = self.write_pos.load(Ordering::Acquire);
        let read_pos = self.read_pos.load(Ordering::Relaxed);

        if read_pos == write_pos {
            return 0.0;
        }

        let sample = unsafe {
            let ptr = self.data.as_ptr();
            *ptr.add(read_pos)
        };

        let next_pos = (read_pos + 1) % RING_BUFFER_SIZE;
        self.read_pos.store(next_pos, Ordering::Release);
        sample
    }

    fn available(&self) -> usize {
        let write_pos = self.write_pos.load(Ordering::Relaxed);
        let read_pos = self.read_pos.load(Ordering::Relaxed);

        if write_pos >= read_pos {
            write_pos - read_pos
        } else {
            RING_BUFFER_SIZE - read_pos + write_pos
        }
    }
}

#[cfg(feature = "audio")]
impl AudioOutput {
    pub fn new() -> Result<Self, String> {
        let (stream, stream_handle) = OutputStream::try_default()
            .map_err(|e| format!("Failed to create audio output: {}", e))?;

        let sink = Sink::try_new(&stream_handle)
            .map_err(|e| format!("Failed to create audio sink: {}", e))?;

        let ring_buffer = std::sync::Arc::new(RingBuffer::new());

        let source = RingBufferSource {
            buffer: std::sync::Arc::clone(&ring_buffer),
            sample_rate: SAMPLE_RATE,
        };

        sink.append(source);

        Ok(AudioOutput {
            _stream: stream,
            sink,
            ring_buffer,
        })
    }

    pub fn play_samples(&mut self, samples: Option<&[f32]>) {
        if let Some(samples) = samples {
            if self.ring_buffer.available() < RING_BUFFER_SIZE - samples.len() - 100 {
                self.ring_buffer.write(samples);
            }
        }
    }

    #[allow(dead_code)]
    pub fn is_playing(&self) -> bool {
        !self.sink.is_paused()
    }
}

#[cfg(feature = "audio")]
struct RingBufferSource {
    buffer: std::sync::Arc<RingBuffer>,
    sample_rate: u32,
}

#[cfg(feature = "audio")]
impl Iterator for RingBufferSource {
    type Item = f32;

    fn next(&mut self) -> Option<Self::Item> {
        Some(self.buffer.read())
    }
}

#[cfg(feature = "audio")]
impl Source for RingBufferSource {
    fn current_frame_len(&self) -> Option<usize> { None }
    fn channels(&self) -> u16 { 1 }
    fn sample_rate(&self) -> u32 { self.sample_rate }
    fn total_duration(&self) -> Option<std::time::Duration> { None }
}

// ============================================================
// スタブ実装（rodioが無効な場合）
// ============================================================

#[cfg(not(feature = "audio"))]
pub struct AudioOutput { _dummy: () }

#[cfg(not(feature = "audio"))]
impl AudioOutput {
    pub fn new() -> Result<Self, String> { Ok(AudioOutput { _dummy: () }) }
    pub fn play_samples(&mut self, _samples: Option<&[f32]>) {}
}
