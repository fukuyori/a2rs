...
impl Gui {
    ...
    /// ツールバーのドラッグ可能エリア上にいるか
    ///
    /// ボタンや音量スライダー上ではなく、空いている背景部分のみを返す。
    pub fn is_in_toolbar_drag_zone(&self, width: usize) -> bool {
        if self.fullscreen || self.mouse_y < 0.0 || self.mouse_y >= TOOLBAR_HEIGHT as f32 {
            return false;
        }

        self.get_button_at(self.mouse_x).is_none() && !self.is_over_volume_slider(width)
    }
...
/// 簡易フォントパターン（6x10ピクセル）
pub(crate) fn get_char_pattern(ch: char) -> [u8; 10] {
    match ch {
...
pub fn draw_text_debug(buffer: &mut [u32], buffer_width: usize, x: usize, y: usize, text: &str, color: u32) {
    draw_text_small(buffer, buffer_width, x, y, text, color);
}
